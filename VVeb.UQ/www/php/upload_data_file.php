...
// --- Get number of files
$n_files = count($_FILES["dataFileToUpload"]["name"]);
if($n_files == 0) 
{
...
for($i=0 ; $i<$n_files ; $i++)
{
  ...
  $file_name = basename($_FILES["dataFileToUpload"]["name"][$i]);
  ...
  $target_file = basename($_FILES["dataFileToUpload"]["name"][$i]);
  ...
  if (move_uploaded_file($_FILES["dataFileToUpload"]["tmp_name"][$i], $target_file))
  {
