...
# We get the external inputs for VVebUQ runs
# Set defaults first
VVebUQ_size = [100, 50]
VVebUQ_energy = 14e6
VVebUQ_particles = 5000
...
file_read.close()
# Replace with values from input file
if (len(values) == 3):
    VVebUQ_size[0] = int(values[0])
    VVebUQ_size[1] = int(float(values[0])/2.0)
    VVebUQ_energy = float(values[1])
    VVebUQ_particles = int(values[2])
...
sett.inactive = 0
sett.particles = VVebUQ_particles
...
source.energy = openmc.stats.Discrete([VVebUQ_energy], [1])
...
mesh.dimension = [VVebUQ_size[0], VVebUQ_size[1], VVebUQ_size[0]]  # width, depth, height
...
os.system('python openmc-statepoint-3d.py -i '+str(sp_filename)+' -t 1 -n tally_on_mesh -m 1 -o tally_on_mesh.vtk')
